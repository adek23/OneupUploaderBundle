...
use Oneup\UploaderBundle\Event\EmptyUploadEvent;
...
abstract class AbstractController
{
	...
	/**
	 *  This function is a helper function which dispatches event when there were no uploaded files
	 *
	 *  @param response A response object.
	 *  @param request The request object.
	 */
	protected function dispatchEmptyUploadEvent(ResponseInterface $response, Request $request)
	{
		$dispatcher = $this->container->get('event_dispatcher');

		// dispatch pre upload event (both the specific and the general)
		$emptyUploadEvent = new EmptyUploadEvent($response, $request, $this->type, $this->config);
		$dispatcher->dispatch(UploadEvents::EMPTY_UPLOAD, $emptyUploadEvent);
		$dispatcher->dispatch(sprintf('%s.%s', UploadEvents::EMPTY_UPLOAD, $this->type), $emptyUploadEvent);
	}
